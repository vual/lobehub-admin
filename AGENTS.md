...
For LobeHub Pro-related UI, every newly added or changed i18n key MUST start with
`LobeHub `. The translated value MUST NOT repeat the `LobeHub` brand name. For
example: `"LobeHub Users": "用户管理"`. Reuse existing generic keys only when
they already exist and do not need to be changed.

Values loaded from database tables or API responses MUST be displayed as data,
without passing them through `t()` and without adding i18n keys for them. Only
static UI labels, actions, and fixed presentation copy should be translated.
