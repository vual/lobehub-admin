...
const defaultPostgresSchema = "admin"
...
func normalizePostgresDSN(dsn string, configuredSchema string) (string, string, error) {
	parsed, err := url.Parse(dsn)
	if err != nil {
	...
	query := parsed.Query()
	schema := strings.TrimSpace(configuredSchema)
	if schema == "" {
		schema = strings.TrimSpace(query.Get("search_path"))
	}
	if schema == "" {
		schema = defaultPostgresSchema
	}
	if len(schema) > 63 {
		return "", "", fmt.Errorf("PostgreSQL schema name %q exceeds 63 bytes", schema)
	}
	for i, char := range schema {
		if (char >= 'a' && char <= 'z') || char == '_' || (i > 0 && char >= '0' && char <= '9') {
			continue
		}
		return "", "", fmt.Errorf("invalid PostgreSQL schema name %q: use lowercase letters, numbers, and underscores, starting with a letter or underscore", schema)
	}

	query.Set("search_path", schema)
	parsed.RawQuery = query.Encode()
	return parsed.String(), schema, nil
}

func ensurePostgresSchema(db *gorm.DB, schema string) error {
...
func chooseDB(envName string, isLog bool) (*gorm.DB, common.DatabaseType, error) {
	...
	if dsn != "" {
		...
		if strings.HasPrefix(dsn, "postgres://") || strings.HasPrefix(dsn, "postgresql://") {
			// Use PostgreSQL
			schemaEnvName := "SQL_SCHEMA"
			if isLog {
				schemaEnvName = "LOG_SQL_SCHEMA"
			}
			dsn, schema, err := normalizePostgresDSN(dsn, os.Getenv(schemaEnvName))
			if err != nil {
				return nil, "", fmt.Errorf("configure %s: %w", envName, err)
			}
			common.SysLog("using PostgreSQL schema " + schema)
			...
			if err := ensurePostgresSchema(db, schema); err != nil {
				return nil, common.DatabaseTypePostgreSQL, err
			}
